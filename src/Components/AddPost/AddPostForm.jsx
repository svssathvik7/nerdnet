...
export default function AddPostForm({ data }) {
  ...
  const handleFormChange = (e) => {
    ...
    if (name === "textPostData" || name === "textTagsData") {
      textActivate();
      setTextTags(
        formData.textTagsData ? formData.textTagsData.split(" ") : []
      );
    } else {
      imgActivate();
  ...
  const [postOption, setPostOption] = useState("user");
  return (
    <div>
      <Header />
      <form
        id="add-post-form"
        className={`bg-white h-fit p-2 absolute left-0 right-0 top-0 bottom-0 m-auto rounded-lg`}
      >
        <div id="post-meta-data" className="flex items-center justify-between">
          <div className="flex items-center justify-center p-2 mx-2">
            <img
              alt="dp"
              src={user ? user.dp : <CgProfile />}
            ...
            <div className="select-none">
              ...
              <p className="text-xs font-light text-slate-600">
                {user ? user.education : "Enthusiast at Nerd.net"}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center justify-center flex-col w-full">
            <div className="flex items-center justify-start w-full m-2">
              <div
                name="text-type"
                className={`${
                  textInput === true ? " scale-105 " : " opacity-70 scale-95 "
                } w-fit mx-1 h-fit p-1 rounded-md cursor-pointer bg-slate-300`}
                onClick={textActivate}
              >
                Text-Post
              </div>
              <div
                name="img-type"
                className={`bg-slate-300 w-fit mx-1 h-fit p-1 rounded-md cursor-pointer ${
                  textInput === false ? " scale-105 " : " opacity-70 scale-95 "
                }`}
                onClick={imgActivate}
              >
                Image-Post
            ...
            <div id="post-input-container" className="w-full">
              {textInput ? (
                <div
                  id="text-input-subform"
                  className="flex items-center justify-center w-full p-2 flex-col"
                >
                  <input
                    name="textPostData"
                    required
                    ...
                    type="text"
                    className="m-2 text-input"
                    value={formData.textPostData}
                ...
                <div id="image-input-subform" className="m-2">
                  <input
                    ...
                    type="url"
                    className=""
                    value={formData.imgPostData}
                    onChange={handleFormChange}
                    autoComplete="off"
                  />
                  <input
                    name="caption"
                    id="image-input-caption"
            ...
            <button
              type="submit"
              id="post-submit"
              className={`bg-yellow-400 p-2 m-2 text-md flex items-center justify-center rounded-full px-8 ${
                disabled ? " opacity-50 bg-slate-400 " : " "
              }`}
            ...
            >
              ...
              {textInput
                ? textTags.map((tag) => (
                    <div
                      key={tag.index}
                      className="bg-slate-400 text-white rounded-3xl p-2 m-2"
                    >
        ...
        <div className="w-fit flex items-center justify-start">
          <p className="w-fit">Post to</p>
          <select
            className="mx-2 select-none outline-none font-bold text-xl"
            id="post-select"
            value={postOption}
            onChange={(e)=>{setPostOption(e.target.value); console.log(postOption)}}
          >
